...
**Follow-up (downstream):** Public `Admin` convenience link for authenticated visitors; admin `View site` link; footer SK external link. `/admin` authorization unchanged (`requireAdminSession()`).
